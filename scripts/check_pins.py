...
gutenberg_kg carries a sibling of this script that DOES check the floors,
because its Dockerfile runs `pip install .` after pinning, so an ARG below its
floor is silently upgraded. This project is `package-mode = false` with no such
step: the ARGs are the last word, and an ARG below the floor already fails the
exact lock-vs-ARG comparison. That sibling also checks runpod/requirements.txt,
which this repo does not have.

docker-compose.yml deliberately carries no version build args: the pins live in
ONE place, the Dockerfile ARGs, so a compose build and `make build-image`
produce the same image. Any `*_VERSION` arg found in compose is reported as a
problem rather than merely compared — a copy that agrees today is one
`poetry update` away from not agreeing, which is how gutenberg_kg drifted to
kgmodule-utils 0.4.6 while its Dockerfile said 0.5.0.
...
def version_key(version: str) -> tuple[int, ...]:
    ...
    Deliberately not ``packaging.version``: this script gates ``make build``
    and should not depend on anything beyond the stdlib. Every version in play
    is plain numeric dotted form; a non-numeric component sorts as 0 rather
    than raising, so a pre-release tag degrades to a loose comparison instead
    of a crash. Shared with the gutenberg_kg sibling.
...
def bump_files(targets: dict[str, str]) -> list[str]:
    ...
    for dist, version in targets.items():
        ...
        changes += rewrite(
            PYPROJECT,
            # The name may carry an extras marker, and poetry writes the floor
            # parenthesised — both `kgmodule-utils[synthesis]>=0.14.0` and
            # `doc-kg (>=0.21.2)` are matched, and group 1 preserves whichever
            # spelling is there.
            re.compile(rf'("{re.escape(dist)}(?:\[[^\]]*\])?\s*\(?\s*>=\s*)([^",\s)]+)'),
            version,
            f"{dist} >=",
        )
        changes += rewrite(
...
def main() -> int:
    ...
    def pypi_cell(dist: str, pinned: str | None) -> str:
        ...
        if version_key(pinned) > version_key(latest):
            # Installable but sorts above PyPI's latest — a pre-release pin, or
            # a newer release that has since been yanked as the latest.
            behind.append(f"{dist}: pinned {pinned}, ahead of PyPI's latest {latest}")
            return f"{latest}  ← pin is ahead"
        return latest
    ...
    # Presence is the problem, not disagreement: a compose-side copy overrides
    # the Dockerfile default at build time, so the pins belong only in the
    # Dockerfile. This catches an arg for a package neither table names, too.
    for arg, value in compose.items():
        problems.append(
            f"docker-compose.yml sets {arg}={value}, overriding the Dockerfile "
            f"default at build time — the pins belong only in the Dockerfile"
        )
